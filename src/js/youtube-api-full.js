...
function initVIdeoBad() {
	...
	swfobject.embedSWF('http://www.youtube.com/apiplayer?enablejsapi=1&version=3&enablejsapi=1&border=0&playerapiid=ytplayer&autoplay=0&showsearch=0&showinfo=0&rel=0&hd=1&autohide=1&color=white&modestbranding=1&fs=0',
...
function initVideo() {
	...
	var atts = { id: "myytplayer" };
	var flashvars = {};
	swfobject.embedSWF(swfUrl, "ytapiplayer", "700", "386", "9", null, 
...
function constructUrl(baseUrl) {
	vid = getParamValue('v') ? getParamValue('v') : first_vid;
	//Descomentar la siguiente linea si queremos pasar chromeless por la URL
	//chromeless = (getParamValue('playerType') && (getParamValue('playerType') === 'chromeless') ? true : false);
	isAS2 = getParamValue('playerVersion') && (getParamValue('playerVersion') == 'as2');
	...
	if (xssSanitizer('Video ID', vid, 'videoId')) {
		if (chromeless) {
			apiUrl = (isAS2) ? baseUrl + 'apiplayer?'+argsString+'&' : baseUrl + 'apiplayer?'+argsString+'version=3&';
		} else if (!isAS2) {
			apiUrl = (argsString) ? baseUrl + 'v/' + vid + '?version=3&' + argsString : baseUrl + 'v/' + vid + '?version=3';

		} else {
			apiUrl = (argsString) ? baseUrl + 'v/' + vid + '?' + argsString : baseUrl + 'v/' + vid;
		}
		return apiUrl;
	}
	// Show a default video ID if the user's URL specified a bad one.
	return baseUrl + 'apiplayer?'+argsString+'&version=3&video_id='+first_vid+'&';
}
...
function updatePlayer() {
	...
	location.href = '/apis/youtube/youtube_player_demo.html?' + 'playerType=' + playerType + '&playerVersion=' + playerVersion;
}
...
function showPlayerParameters(displayStatus) {
	...
	if (originalParameters && definedPlayers[originalParameters] && useOriginalParameters) {
	...
	for (var count in playerParameters) {
		...
		if (parameterName && parameterDefinition && parameterLink) {
			if ((playerVersion && unsupported[playerVersion] && unsupported[playerVersion][playerParameters[count]]) || (parameterName.id == 'loopas2' && displayStatus == 'load')) {
...
function updateColor(box) {
	...
	if (result != null) {
		...
		if (document.getElementById(box.id + '-preview')) {
			document.getElementById(box.id + '-preview').style.backgroundColor = colorValue;
		}
	}
}
...
function onYouTubePlayerReady(playerId) {
	...
	if (playerId && playerId != 'undefined') {
		...
		if (ytplayer) {
			...
			if (vid && chromeless) {
				cueVideo(vid, 0, 'large');
			} else {
				loadVideo(vid, 0, 'large');
			}
			...
			ytplayer.addEventListener('onPlaybackQualityChange', 'onytplayerQualityChange');
...
function onytplayerStateChange(newState) {
	//$('#testid').hide();
	alert('change');
	events.push('onStateChange event: Player state changed to: "' + newState + '" (' + getPlayerState(newState) + ')');
...
function onytplayerQualityChange(newQuality) {
	events.push('onPlaybackQualityChange event: ' + 'Playback quality changed to "' + newQuality + '"');
...
function loadVideo(idOrUrl, startSeconds, quality) {
	...
	if (xssSanitizer('Video ID or URL', idOrUrl, 'videoIdOrUrl') && xssSanitizer('Start at', startSeconds, 'digits') && xssSanitizer('Suggested quality', quality, 'qualitylevels')) {
		var urlRegex = '/http\:/';
		if (idOrUrl.match(urlRegex)) {
			ytplayer.loadVideoByUrl(idOrUrl, parseInt(startSeconds), quality);
			events.push('loadVideoByUrl(' + idOrUrl + ', parseInt(' + startSeconds + '), ' + quality + ');');
		} else {
...
function cueVideo(idOrUrl, startSeconds, quality) {
	...
	if (xssSanitizer('Video ID or URL', idOrUrl, 'videoIdOrUrl') && xssSanitizer('Start at', startSeconds, 'digits') && xssSanitizer('Suggested quality', quality, 'qualitylevels')) {
		var urlRegex = /http\:/;
		if (idOrUrl.match(urlRegex)) {
			ytplayer.cueVideoByUrl(idOrUrl, parseInt(startSeconds), quality);
			events.push('cueVideoByUrl(' + idOrUrl + ', parseInt(' + startSeconds + '), ' + quality + ');');
		} else {
...
function setQuality(newQuality) {
	...
	ytplayer.setPlaybackQuality(newQuality);
}
...
function updateytplayerInfo() {
	...
	updateHTML('playerstate', getDuration());
	
	var perc= parseInt((getCurrentTime()/getDuration())*100);
 ...
 * calls ytplayer.getPlayerState() to retrieve an integer, which
	...
	if (result) {
		...
		if (updateParams) {
			...
			for (var i = 0, input; input = inputs[i]; i++) {
				...
				if ((!isAS2 && as2Options[paramName]) || (isAS2 && as3Options[paramName])) {
					continue;
				}
				...
				if (paramName == 'width' && xssSanitizer('Width', paramValue, 'digits')) {
					...
					result = result.replace(/ width=\"([^\"]+)/g, ' width=\"' + width);
					continue;
					...
					result = result.replace(/ height=\"([^\"]+)/g, ' height=\"' + height);
					continue;
				} else if (paramName == 'color1' || paramName == 'color2') {
					paramValue = '0x' + paramValue;
				} else if (paramName == 'rel' || paramName == 'showsearch' || paramName == 'showinfo' || paramName == 'controls') {
					if (paramValue == 'on') {
						continue;
					}
					paramValue = '0';
			...
			result = result.replace(/www.youtube.com([^\"]+)/g, 'www.youtube.com$1' + argString);
		}
